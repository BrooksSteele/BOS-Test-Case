...
def detect_rr_layout(ws):
    """
    Auto-detect rent roll column layout by reading the header row.
    Returns (header_row_idx, col_map, is_multirow) where col_map maps
    field names to 0-based column indices.
    """
    # Look for a row containing 'Unit' and 'Floorplan' in first 10 rows
    for i in range(1, 11):
        row = list(ws.iter_rows(min_row=i, max_row=i, values_only=True))[0]
        row_lower = [str(v or "").strip().lower() for v in row]
        if any("unit" in v and "floorplan" not in v and v != "unit designation"
               for v in row_lower if v):
            # Found header row — map columns
            col_map = {}
            for j, h in enumerate(row_lower):
                if h in ("unit", "unit number", "unit #") and "col_unit" not in col_map:
                    col_map["unit"] = j
                if h in ("floorplan", "floor plan", "unit type") and "fp" not in col_map:
                    col_map["fp"] = j
                if h in ("sqft", "sq ft", "sq. ft.", "size", "net sf") and "sqft" not in col_map:
                    col_map["sqft"] = j
                if ("status" in h or "lease status" in h) and "status" not in col_map:
                    col_map["status"] = j
                if "market" in h and "rent" in h and "mkt" not in col_map:
                    col_map["mkt"] = j
                if "lease end" in h or ("lease" in h and "exp" in h):
                    col_map["lease"] = j
            if "unit" in col_map and "fp" in col_map:
                unit_col_idx = col_map["unit"]
                # Accept both integer and string unit numbers when detecting multirow
                all_rows = list(ws.iter_rows(min_row=i+1, max_row=i+25, values_only=True))
                unit_indices = [j for j,r in enumerate(all_rows)
                                if r[unit_col_idx] and str(r[unit_col_idx]).strip()
                                and str(r[unit_col_idx]).strip().lower() not in
                                ("unit","unit number","floorplan","n/a","")]
                is_multirow = len(unit_indices) < 2 or (unit_indices[1] - unit_indices[0]) > 2
                col_map.setdefault("sqft",   6)
                col_map.setdefault("status", 7)
                col_map.setdefault("mkt",    18)
                col_map.setdefault("lease",  16 if is_multirow else 11)
                return i, col_map, is_multirow
    # Fallback: Market Station positional layout (one row per unit)
    return 6, {"unit":2,"fp":3,"sqft":5,"status":6,"mkt":12,"lease":11}, False
...
def parse_rent_roll(rr_bytes, total_units):
    ...
    header_row, col_map, is_multirow = detect_rr_layout(ws)
    unit_col   = col_map["unit"]
    fp_col     = col_map["fp"]
    sqft_col   = col_map["sqft"]
    status_col = col_map["status"]
    mkt_col    = col_map["mkt"]
    lease_col  = col_map["lease"]
    # Charge code col — always col 19 in Yardi multi-row format
    charge_col = 19
    ...
    for row in ws.iter_rows(min_row=header_row + 1, values_only=True):
        ...
        fp   = row[fp_col]   if fp_col   < len(row) else None

        if is_multirow:
            # New unit header row: has a unit number (int/float)
            if unit and isinstance(unit, (int, float)) and fp and isinstance(fp, str):
                status = str(row[status_col] if status_col < len(row) else "").strip()
                if status in ("Pending renewal", "Applicant", "Pending resident"):
                    current = None; continue
                le_raw = row[lease_col] if lease_col < len(row) else None
                mkt    = row[mkt_col]   if mkt_col   < len(row) else None
                current = {
                    "unit":        unit,
                    "floorplan":   str(fp).strip(),
                    "sqft":        int(row[sqft_col]) if sqft_col < len(row) and row[sqft_col] else 0,
                    "status":      status,
                    "market_rent": float(mkt) if mkt else 0,
                    "actual_rent": 0,
                    "lease_end":   parse_date_val(le_raw),
                }
                if unit not in unit_dict:
                    unit_dict[unit] = current
            # Sub-row: look for RENT charge code
            elif current and charge_col < len(row):
                charge = str(row[charge_col] or "")
                if "RESIDENT" in charge and "RENT" in charge and "AMENITY" not in charge                    and "PARKING" not in charge and "PET" not in charge                    and "STORAGE" not in charge and "TRASH" not in charge                    and "PEST" not in charge and "PACKAGE" not in charge:
                    m = re.search(r"RENT\s+([\d,]+\.?\d*)", charge)
                    if m:
                        amt = float(m.group(1).replace(",", ""))
                        if amt > 0 and unit_dict.get(current["unit"]):
                            unit_dict[current["unit"]]["actual_rent"] = amt
        else:
            # One row per unit (Market Station style)
            status = str(row[status_col] if status_col < len(row) else "").strip()
            ...
            if unit_key in unit_dict: continue
            mkt = row[mkt_col] if mkt_col < len(row) else None
            # Market Station stores RENT charge in col 18 directly
            act = row[18] if 18 < len(row) else None
            unit_dict[unit_key] = {
                "unit":        unit_key,
                "floorplan":   str(fp).strip(),
                "sqft":        int(row[sqft_col]) if sqft_col < len(row) and row[sqft_col] else 0,
                "status":      status,
                "market_rent": float(mkt) if mkt else 0,
                "actual_rent": float(act) if act else 0,
...
def run_pipeline(t12_bytes, rr_bytes, property_name, total_units, t12_sheet):
    ...
    if rr_bytes:
        try:
            ...
            import traceback
            warnings.append(f"⚠ Rent roll error: {type(e).__name__}: {e}. Rent Schedule not populated.")
            warnings.append(f"Detail: {traceback.format_exc().splitlines()[-1]}")
...
if t12_file:
    try:
        ...
        if len(sheets) > 1:
            # Prefer 'Report1' if present, otherwise default to last sheet
            # (Yardi files often have a 'JP' summary sheet first and 'Report1' second)
            preferred = next((s for s in sheets if s.lower() in
                             ("report1","report 1","detail","operating statement")), None)
            default_idx = sheets.index(preferred) if preferred else len(sheets) - 1
            sel = st.selectbox("Data sheet", sheets, index=default_idx)
